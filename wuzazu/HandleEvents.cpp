...
void EventHandler_::clickInactivePlayer(Cell* clickedCell)
{
	...
	int temp = 0;
	while (!cellQueue.empty() && temp < 21)
	{
		temp++;
		currCell = cellQueue.front();
		cellQueue.pop_front();

		if (currCell->N() && (int)weights[currCell->N()].val() < (int)(weights[prevCell].val() - currCell->N()->terrain().weight())) //going from -1 -> 4 or 3 -> 4
		...
		if (currCell->S() && (int)weights[currCell->S()].val() < (int)(weights[prevCell].val() - currCell->S()->terrain().weight()))
		...
		if (currCell->E() && (int)weights[currCell->E()].val() < (int)(weights[prevCell].val() - currCell->E()->terrain().weight()))
		...
		if (currCell->W() && (int)weights[currCell->W()].val() < (int)(weights[prevCell].val() - currCell->W()->terrain().weight()))
